...
export class ListaProdutosComponent implements OnInit {
    ...
    switch (this.ordenacaoSelecionada) {
      ...
      case 'price-asc':
        produtosOrdenados.sort(
          (a, b) => this.getPrice(a.price) - this.getPrice(b.price)
        );
        break;
      case 'price-desc':
        produtosOrdenados.sort(
          (a, b) => this.getPrice(b.price) - this.getPrice(a.price)
        );
        break;
      case 'name-asc':
        produtosOrdenados.sort((a, b) =>
          this.getProductName(a).localeCompare(this.getProductName(b))
        );
        break;
      case 'name-desc':
        produtosOrdenados.sort((a, b) =>
          this.getProductName(b).localeCompare(this.getProductName(a))
        );
        break;
      case 'stock-only':
        // Filtrar produtos com estoque da lista atual
        const produtosComEstoque = produtosOrdenados.filter((produto) =>
          this.hasStock(produto)
        );
  ...
  private definirMensagemSemProdutos(): void {
    switch (this.ordenacaoSelecionada) {
      case 'stock-only':
        ...
        this.descricaoSemProdutos =
          'Todos os produtos estão sem estoque no momento. Tente outros filtros ou volte mais tarde.';
        break;
      case 'price-asc':
      case 'price-desc':
        this.mensagemSemProdutos =
          'Nenhum produto encontrado para ordenação por preço';
        this.descricaoSemProdutos =
          'Tente usar outros filtros ou verificar se há produtos disponíveis.';
        break;
      case 'name-asc':
      case 'name-desc':
        this.mensagemSemProdutos =
          'Nenhum produto encontrado para ordenação por nome';
        this.descricaoSemProdutos =
          'Tente usar outros filtros ou verificar se há produtos disponíveis.';
        break;
      default:
  ...
  onImageError(event: any, produto: IProdutos) {
    ...
    produto.imageSafe = this.imageService.sanitizeImageUrl(
  ...
  hasStock(produto: IProdutos): boolean {
    ...
    if (typeof produto.stock === 'string') {
      ...
      if (
        stockStr.includes('disponível') ||
        stockStr.includes('sim') ||
        stockStr.includes('yes')
      ) {
        return true;
      }
      ...
      if (
        stockStr.includes('indisponível') ||
        stockStr.includes('não') ||
        stockStr.includes('no') ||
        stockStr.includes('sem')
      ) {
        return false;
      }
