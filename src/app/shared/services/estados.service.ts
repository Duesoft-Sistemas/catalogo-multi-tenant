...
export class EstadosService {
  ...
  constructor(private http: HttpClient) {
    this.urlBase = `${environment?.UrlBase}/Estados`;
  }

  getAll(hiddenError?: boolean): Promise<IEstados> {
  ...
  getById(codigo: number): Promise<IEstados> {
    return new Promise((resolve, reject) => {
  ...
  getByUf(uf: string): Promise<IEstados> {
    return new Promise((resolve, reject) => {
      this.httpGetByUf(uf)
        .pipe(take(1))
        .subscribe({
  ...
  update(codigo: number, data: IEstados): Promise<boolean> {
    return new Promise((resolve, reject) => {
  ...
  private httpGetById(id: number): Observable<IEstados> {
    return this.http.get<IEstados>(`${this.urlBase}/${id}`);
  }

  private httpGetByUf(uf: string): Observable<IEstados> {
    return this.http.get<IEstados>(`${this.urlBase}/GetByUf/${uf}`);
  }

  private httpUpdate(id: number, record: IEstados): Observable<Response> {
